...
class HybridCollabFilter():
    def __init__(self, numUsers, embedding_dim, input_dim,seq_len,word_embed,embed_mat, img_feats,reg_l = .001):
        ...
        featTensor =  tflearn.fully_connected(featTensor,self.h_dense,activation='relu' )
        featTensor = tflearn.dropout(featTensor, 0.8)
        featTensor =  tflearn.fully_connected(featTensor,embedding_dim,activation='linear')
    ...
    def train(self, users, movies, ratings, featMat,wordSeq,imgMat, eval_type='AUC', val_freq=5):
        ...
        for i in range(self.epochs):
            ...
            save_path = self.saver.save(self.session, "model2.ckpt")
