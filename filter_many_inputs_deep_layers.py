...
class HybridCollabFilter():
    ...
    def __init__(self, numUsers, numMovies, inputdim_image = None, inputdim_meta = None, 
        ...
        self.movie_size = movieTensor.get_shape()[1]
        ...
        self.W_hidden_2 = tf.Variable(self.init_var *
                                      tf.random_normal([edim_hidden_1 + 1, edim_hidden_2]))
        self.W_hidden_output = tf.Variable(self.init_var *
                                      tf.random_normal([edim_hidden_2 + 1,
        ...
        self.act_hidden_1 = tf.concat(1, [sig_hidden_1, tf.ones(self.height)])
        ...
        self.act_hidden_2 = tf.concat(1, [sig_hidden_2, tf.ones(self.height)])
        
        self.fullyconnectedoutput = tf.reduce_sum(tf.mul(self.act_hidden_2, self.W_hidden_output) , 1)
        self.sig_hidden_output = tf.sigmoid(self.fullyconnectedoutput)
        self.act_hidden_output = tf.concat(1, [sig_hidden_output, tf.ones(self.height)])
